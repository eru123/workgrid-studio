// PostgreSQL driver — stub. To be implemented with deadpool-postgres + tokio-postgres.
// The trait surface is fixed here so the rest of the backend compiles; calling
// any method returns a not_implemented error.
...
macro_rules! unimpl {
    ($name:literal) => {
        Err(AppError::not_implemented(concat!($name, " is not implemented for PostgreSQL yet")))
    };
}
