...
import {
    Play, Square, Trash2, Copy, Download, Loader2,
    Server, Database, Clock, CheckCircle2, XCircle, AlertCircle,
    FileText, WrapText
} from "lucide-react";
...
export function QueryTab({ tabId: _tabId, profileId, profileName, database: initialDatabase }: Props) {
    // ── State ──────────────────────────────────────────────────
    ...
    const handleRun = useCallback(async () => {
        ...
        try {
            ...
            const filteredResults = selectedDb
                ? res.filter((_, i) => i > 0)
                : res;
    ...
    return (
        <div ref={containerRef} className="flex flex-col w-full h-full bg-background text-foreground text-xs overflow-hidden">
            ...
            {/* ─── SQL Editor area ─────────────────────────── */}
            <div className="shrink-0 flex flex-col overflow-hidden" style={{ height: `${splitPercent}%` }}>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                {error && (
                    <div className="flex items-start gap-2 px-3 py-2 bg-red-500/10 text-red-400 border-b shrink-0">
                        ...
                        <pre className="text-xs font-mono whitespace-pre-wrap flex-1 select-text">{error}</pre>
                    </div>
                )}
                        ...
                        <table className="w-full text-xs border-collapse">
                            ...
                            <tbody>
                                {activeResult.rows.map((row, ri) => (
                                    <tr key={ri} className="border-b hover:bg-accent/20 transition-colors">
                                        ...
                                        {row.map((val, ci) => (
                                            <td
                                                ...
                                                className={cn(
                                                    ...
                                                    wordWrap ? "whitespace-pre-wrap break-all" : "whitespace-nowrap truncate"
                                                )}
            ...
            <div className="flex items-center gap-3 px-3 py-1 border-t bg-muted/20 shrink-0 text-[10px]">
                <div className={cn(
                    ...
                    running ? "text-yellow-400" : error ? "text-red-400" : results.length > 0 ? "text-green-400" : "text-muted-foreground"
                )}>
                    {running ? <Loader2 className="w-3 h-3 animate-spin" /> :
                        error ? <XCircle className="w-3 h-3" /> :
                            results.length > 0 ? <CheckCircle2 className="w-3 h-3" /> :
                ...
                {activeResult && activeResult.rows.length > 0 && (
                    <div className="text-muted-foreground">
                        {activeResult.rows.length} row(s) × {activeResult.columns.length} col(s)
                    </div>
                )}
            </div>
        </div>
