            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {(Object.keys(DB_TYPE_LABELS) as DatabaseType[]).map((type) => {
                  const isSupported = type === "mysql" || type === "mariadb";
                  return (
                    <button
                      key={type}
                      onClick={() => handleTypeChange(type)}
                      className={cn(
                        "flex flex-col items-center gap-1 py-2.5 px-2 rounded border text-xs transition-all relative overflow-hidden",
                        formData.type === type
          ...
          <div className="p-6 max-w-xl">
            ...
            <div className="border rounded-lg">
              ...
              <div className="divide-y divide-border/50">
                <DetailRow
                  label="Type"
                  value={DB_TYPE_LABELS[selectedProfile.type]}
                />
                {selectedProfile.type === "sqlite" ? (
                  <DetailRow
                    label="File Path"
                    value={selectedProfile.filePath || "—"}
                  />
                ) : (
                  <>
                    <DetailRow
                    ...
                    <DetailRow
                      label="Port"
                      value={
                        selectedProfile.port != null
                          ? String(selectedProfile.port)
                          : "—"
                      }
                    />
                    <DetailRow
                      label="Username"
                      value={selectedProfile.user || "—"}
                    />
                    <DetailRow
                      label="Password"
                    ...
                    <DetailRow
                      label="Database"
                      value={selectedProfile.database || "—"}
                    />
                    <DetailRow
                      label="SSL"
                      value={selectedProfile.ssl ? "Enabled" : "Disabled"}
                    />
                  </>
                )}
