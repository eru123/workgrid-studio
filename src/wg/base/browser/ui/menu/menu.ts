...
export class Menu extends ActionBar {
	...
	private styleScrollElement(scrollElement: HTMLElement, style: IMenuStyles): void {
		...
		const borderRadius = 'var(--wg-cornerRadius-large)';
...
export function getMenuWidgetCSS(style: IMenuStyles, isForShadowDom: boolean): string {
	const borderColor = style.borderColor ?? 'var(--wg-menu-border)';
	let result = /* css */`
.monaco-menu {
	font-size: 13px;
	border-radius: var(--wg-cornerRadius-large);
...
.monaco-menu .monaco-action-bar .action-item.disabled .action-label:hover {
	color: var(--wg-disabledForeground);
}
...
.monaco-menu .monaco-action-bar.vertical .action-label.separator {
	display: block;
	border-bottom: 1px solid var(--wg-menu-separatorBackground);
	padding-top: 1px;
	padding: 30px;
}
...
.monaco-menu .monaco-action-bar.vertical .action-menu-item {
	...
	position: relative;
	margin: 0 4px;
	border-radius: var(--wg-cornerRadius-medium);
}
...
.context-view.monaco-menu-container {
	...
	box-shadow: var(--wg-shadow-lg${style.shadowColor ? `, 0 0 12px ${style.shadowColor}` : ''});
	border-radius: var(--wg-cornerRadius-large);
	overflow: hidden;
}
...
:host-context(.hc-light) .monaco-menu.monaco-menu .monaco-action-bar.vertical .action-item.focused .action-menu-item {
	outline: 1px solid var(--wg-menu-selectionBorder) !important;
