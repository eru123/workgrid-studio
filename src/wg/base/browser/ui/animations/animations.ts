...
export function triggerConfettiAnimation(element: HTMLElement) {
	...
	ring.style.border = '2px solid var(--wg-focusBorder, #007acc)';
...
export function triggerFloatingIconsAnimation(element: HTMLElement, icon: ThemeIcon) {
	...
	for (let i = 0; i < iconCount; i++) {
		...
		iconEl.style.color = 'var(--wg-focusBorder, #007acc)';
...
export function triggerPulseWaveAnimation(element: HTMLElement) {
	...
	glow.style.backgroundColor = 'var(--wg-focusBorder, #007acc)';
...
export function triggerRadiantLinesAnimation(element: HTMLElement) {
	...
	for (let i = 0; i < 8; i++) {
		...
		dot.style.height = `${size}px`;
		dot.style.borderRadius = '50%';
		dot.style.backgroundColor = 'var(--wg-editor-foreground, #ffffff)';
		dot.style.left = `${cx - size / 2}px`;
		dot.style.top = `${cy - size / 2}px`;
		overlay.appendChild(dot);
	...
	for (let i = 0; i < 8; i++) {
		...
		line.style.backgroundColor = 'var(--wg-focusBorder, #007acc)';
		line.style.left = '-1px';
		line.style.top = '-22px';
